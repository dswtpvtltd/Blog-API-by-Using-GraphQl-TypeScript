import { ApolloCache, gql } from '@apollo/client';
import { ChevronDownIcon, ChevronUpIcon } from '@chakra-ui/icons';
import { Box, Flex, IconButton } from '@chakra-ui/react';
import React, { useState } from 'react';
import { PostSnippetFragment, useVoteMutation, VoteMutation } from '../generated/graphql';

interface UpdootSectionProps {
	post: PostSnippetFragment;
}

const updateAfterVote = (value: number, postId, cache: ApolloCache<VoteMutation>) => {
	const data = cache.readFragment<{
		id: number;
		points: number;
		voteStatus: number;
	}>({
		id: 'Post:' + postId,
		fragment: gql`
			fragment _ on Post {
				id
				points
				voteStatus
			}
		`,
	});
	if (data) {
		if (data.voteStatus === value) {
			return;
		}

		const newPoints = (data.points as number) + (!data.voteStatus ? 1 : 2) * value;

		cache.writeFragment({
			id: 'Post:' + postId,
			fragment: gql`
				fragment __ on Post {
					points
					voteStatus
				}
			`,
			data: { points: newPoints, voteStatus: value } as any,
		});
	}
};

const UpdootSection: React.FC<UpdootSectionProps> = ({ post }) => {
	const [loadingState, setLoadingState] =
		useState<'updoot-loading' | 'downdoot-loading' | 'not-loading'>('not-loading');
	const [vote] = useVoteMutation();
	return (
		<Flex direction="column" w="50px" alignItems="center" justifyContent="center">
			<Box w="30px" textAlign="center">
				<IconButton
					style={{ backgroundColor: post.voteStatus === 1 ? 'green' : undefined }}
					isLoading={loadingState === 'updoot-loading'}
					aria-label="updoot post"
					name="chevron-up">
					<ChevronUpIcon
						style={{ color: 'white' }}
						onClick={async () => {
							if (post.voteStatus === 1) {
								return;
							}
							setLoadingState('updoot-loading');
							await vote({
								variables: {
									postId: post.id,
									value: 1,
								},
								update: (cache) => updateAfterVote(1, post.id, cache),
							});
							setLoadingState('not-loading');
						}}
					/>
				</IconButton>
			</Box>
			<Box w="30px" textAlign="center">
				{post.points}
			</Box>
			<Box w="30px" textAlign="center">
				<IconButton
					style={{ backgroundColor: post.voteStatus === -1 ? 'tomato' : undefined }}
					isLoading={loadingState === 'downdoot-loading'}
					aria-label="downdoot post"
					name="chevron-down">
					<ChevronDownIcon
						style={{ color: 'white' }}
						onClick={async () => {
							if (post.voteStatus === -1) {
								return;
							}
							setLoadingState('downdoot-loading');
							await vote({
								variables: {
									postId: post.id,
									value: -1,
								},
								update: (cache) => updateAfterVote(-1, post.id, cache),
							});
							setLoadingState('not-loading');
						}}
					/>
				</IconButton>
			</Box>
		</Flex>
	);
};

export default UpdootSection;
